...
void DXApp::InitProjectionMatrix(Shape::Mesh * mesh)
{
	...
	// 투영 행렬.
	projection = XMMatrixPerspectiveFovLH(fovY, (float)clientWidth / (float)clientHeight, 1.0f, 100000.0f);
}
...
bool DXApp::InitWVPBuffer(Shape::Mesh * mesh)
{
	...
	cbDesc.ByteWidth = sizeof(ShaderUtil::CBPerObject);
...
bool DXApp::InitScene()
{
	...
	for (int ix = 0; ix < meshes.size(); ++ix)
	{
		...
		// 래스터라이저 스테이트 생성.
		if (!meshes[ix].vertexShader.CreateRasterizerState(pDevice, D3D11_FILL_SOLID, D3D11_CULL_BACK)) return false;
...
void DXApp::InitMeshInfo()
{
	...
	LPCSTR fbxCardboard = "Resources//Model//SK_CharM_Cardboard.fbx";
	LPCSTR fbxFrostGiant = "Resources//Model//SK_CharM_FrostGiant.fbx";

	ShaderUtil::Texture tppDiffuseMap;
	tppDiffuseMap.fileName = L"Resources//Textures//T_Chr_FPS_D.png";

	ShaderUtil::Texture tppNormalMap;
	tppNormalMap.fileName = L"Resources//Textures//T_Chr_FPS_N.png";

	ShaderUtil::Texture cardDiffuseMap;
	cardDiffuseMap.fileName = L"Resources//Textures//Char_M_Cardboard_D.png";

	ShaderUtil::Texture cardNormalMap;
	cardNormalMap.fileName = L"Resources//Textures//Char_M_Cardboard_N.png";

	ShaderUtil::Texture frostDiffuseMap;
	frostDiffuseMap.fileName = L"Resources//Textures//T_Char_M_FrostGiant_D.png";

	ShaderUtil::Texture frostNormalMap;
	frostNormalMap.fileName = L"Resources//Textures//T_Char_M_FrostGiant_N.png";
	...
	ShaderUtil::Texture earthTexture;
	earthTexture.fileName = L"Resources//Textures//earth_daymap.jpg";
	
	ShaderUtil::Texture earthNormalMap;
	earthNormalMap.fileName = L"Resources//Textures//earth_normal_map.png";
	...
	// 정점 쉐이더.
	sphereMesh.vertexShader = ShaderUtil::VertexShader(L"Shaders//LabertianDiffuseVS.fx", "main", "vs_5_0");
	sphereMesh.pixelShader = ShaderUtil::PixelShader(L"Shaders//LabertianDiffusePS.fx", "main", "ps_5_0");

	// 픽셀 쉐이더 쪽에서 사용하는 텍스처 추가.
	sphereMesh.pixelShader.pTextures.push_back(earthTexture);
	sphereMesh.pixelShader.pTextures.push_back(earthNormalMap);

	//트랜스폼 설정
	sphereMesh.position = XMFLOAT3(0.0f, 0.0f, 0.0f);
	sphereMesh.rotation = XMFLOAT3(0.0f, 0.0f, 0.0f);
	sphereMesh.scale = XMFLOAT3(1.0f, 1.0f, 1.0f);

	// 엔진에서 렌더링하는 메쉬 배열에 추가.
	...
	Shape::Mesh tppMesh;
	// FBX 파일 이름 설정.
	tppMesh.fbxName = fbxNameTPP;
	// 정점 쉐이더 / 픽셀 쉐이더 설정 (쉐이더 파일 경로, 메인함수 이름, 쉐이더 프로파일 이름 설정).
	tppMesh.vertexShader = ShaderUtil::VertexShader(L"Shaders//HalfLabertianDiffuseVS.fx", "main", "vs_5_0");
	tppMesh.pixelShader = ShaderUtil::PixelShader(L"Shaders//HalfLabertianDiffusePS.fx", "main", "ps_5_0");
	// 픽셀 쉐이더 쪽에서 사용하는 텍스처 추가.
	tppMesh.pixelShader.pTextures.push_back(tppDiffuseMap);
	// 위치 설정.
	tppMesh.position = XMFLOAT3(150.0f, -120.0f, 0.0f);

	// 엔진에서 렌더링 하는 메쉬 배열에 메쉬 추가.
	//meshes.push_back(tppMesh);
