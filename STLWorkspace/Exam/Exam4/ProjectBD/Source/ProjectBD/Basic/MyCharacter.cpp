...
#include "Item/UI/ItemSlotWidgetBase.h"
// Sets default values
...
void AMyCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction(TEXT("Inventory"), IE_Released, this, &AMyCharacter::OpenInventory);
	PlayerInputComponent->BindAction(TEXT("GetItem"), IE_Released, this, &AMyCharacter::GetItem);
}
...
void AMyCharacter::GetItem()
{
	...
	if (PickupItem && !PickupItem->IsPendingKill())
	{
		...
		if (PC)
		{
			...
			{
				UItemSlotWidgetBase* Slot = PC->Inventory->GetEmptySlot();
				if (Slot)
				{
...
float AMyCharacter::TakeDamage(float DamageAmount, FDamageEvent const & DamageEvent, AController * EventInstigator, AActor * DamageCauser)
{
	...
	{
		//DamageEvent
		UE_LOG(LogClass, Warning, TEXT("d맞앋ㅆ따"));
		CurrentHP -= DamageAmount;
	}
...
void AMyCharacter::ViewItemTooltip()
{
	...
	if (ClosestItem)
	{
		PC->ItemTooltip->ItemName->SetText(FText::FromString(ClosestItem->ItemDataTableComponent->GetItemData(ClosestItem->ItemIndex).ItemName));
