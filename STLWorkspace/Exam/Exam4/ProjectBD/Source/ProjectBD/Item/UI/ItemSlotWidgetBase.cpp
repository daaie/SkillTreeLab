...
void UItemSlotWidgetBase::NativeConstruct()
{
	...
	DataTable = AssetLoader.LoadSynchronous<UDataTable>(FStringAssetReference(TEXT("DataTable'/Game/Blueprints/Data/ItemDataTable.ItemDataTable'")));
...
void UItemSlotWidgetBase::SetItem(int NewItemIndex)
{
	...
	{
		UE_LOG(LogClass, Warning, TEXT("Debug %d"), NewItemIndex);
		if (ItemThumb)
		{
			FStreamableManager AssetLoader;
...
{
	UE_LOG(LogClass, Warning, TEXT("Debug4443 "));
	if (DataTable)
	{
		if (Data.ItemIndex != 0)
		{
