...
DXApp::~DXApp()
{
	...
	Memory::SafeRelease(pCubeMapTexture);
...
void DXApp::InitVertexBuffer()
{
	...
	// FBX 로드
	HRESULT hr = LoadFBX("HeroTPP.FBX", &pVertices);
	if (FAILED(hr))
		return;
...
void DXApp::InitTransformMatrix()
{
	...
	float rotY = XMConvertToRadians(90.0f);
	...
	// 카메라 위치.
	cameraPos = XMVectorSet(-100.0f, 180.0f, 0.0f, 0.0f);
...
bool DXApp::InitScene()
{
	...
	{
		OutputDebugString(L"디퓨즈 텍스쳐 로드 실패");
		OutputDebugString(L"\n");
		return false;
	...
	{
		OutputDebugString(L"큐브맵 텍스쳐 로드 실패");
		OutputDebugString(L"\n");
		return false;
...
bool DXApp::LoadTexture(LPCWSTR fileName, ID3D11ShaderResourceView ** pOutTexure, ID3D11SamplerState ** poutsamplerstate)
{
	HRESULT hr;
	// 텍스처 파일 로드.
	hr = D3DX11CreateShaderResourceViewFromFile(pDevice,
		fileName, NULL, NULL, pOutTexure, NULL);
