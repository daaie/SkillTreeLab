...
bool ShaderUtil::CompileShader(LPCWSTR shader, LPCSTR entry, LPCSTR profile, ID3DBlob ** pOutShaderBuffer)
{
	...
	// 이게 컴파일 ㅏㅎ면 끝이아니라 버퍼에 바이너리로 저장됨. 
	if (FAILED(hr))
	{
...
bool ShaderUtil::BindVertexShader(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, const void * pShaderBytecode, SIZE_T BytecodeLength, ID3D11ClassLinkage * pClassLinkage, ID3D11VertexShader ** ppVertexShader)
{
	// 정점 쉐이더 오브젝트 생성.
	// 생성된 이진버퍼로 오브젝트 생성
...
bool ShaderUtil::BindPixelShader(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, const void * pShaderBytecode, SIZE_T BytecodeLength, ID3D11ClassLinkage * pClassLinkage, ID3D11PixelShader ** ppPixelShader)
{
	// 픽셀 쉐이더 오브젝트 생성.
