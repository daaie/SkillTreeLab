...
{
	// FBX 로드.
	HRESULT hr = LoadFBX("cow.fbx", &pVertices);
	if (FAILED(hr))
		return;
...
void DXApp::InitTransformMatrix()
{
	...
	XMMATRIX translation = XMMatrixTranslation(0.0f, -130.0f, 100.0f);

	world *= rotationX * rotationY * translation;
	//world *= rotationX;

	// 카메라 위치.
	cameraPos = XMVectorSet(0.0f, 90.0f, -70.0f, 0.0f);
...
bool DXApp::InitScene()
{
	...
	if (!LoadTexture(L"T_Chr_FPS_D.png", &pTexture, &pTextureSamplerState))
	{
		OutputDebugString(L"디퓨즈 텍스처 로드 실패");
		OutputDebugString(L"\n");
		return false;
	}
	...
	// 노멀맵 텍스처 로드.
	if (!LoadTexture(L"T_Chr_FPS_N.png", &pNormalTexture, NULL))
	{
		OutputDebugString(L"노멀 텍스처 로드 실패");
		OutputDebugString(L"\n");
		return false;
	}
	...
	// 큐브맵 텍스처 로드.
	if (!LoadTexture(L"cubemap.dds", &pCubemapTexture, NULL))
	{
		OutputDebugString(L"큐브맵 텍스처 로드 실패");
		OutputDebugString(L"\n");
		return false;
	}

	// 텍스처 바인딩.
	pDeviceContext->PSSetShaderResources(2, 1, &pCubemapTexture);
...
bool DXApp::LoadTexture(LPCWSTR fileName, ID3D11ShaderResourceView ** pOutTexture, ID3D11SamplerState ** pOutSamplerState)
{
	HRESULT hr;
	// 텍스처 파일 로드.
	hr = D3DX11CreateShaderResourceViewFromFile(pDevice, 
		fileName, NULL, NULL, pOutTexture, NULL);

	if (FAILED(hr))
		return false;

	// NULL인지 확인.
	if (pOutSamplerState != NULL)
	{
		// 샘플러 상태 서술자 설정.
		D3D11_SAMPLER_DESC samplerDesc;
		ZeroMemory(&samplerDesc, sizeof(D3D11_SAMPLER_DESC));
	...
	for (int ix = 0; ix < childCount; ++ix)
	{
		...
		for (int jx = 0; jx < polygonCount; ++jx)
		{
			...
			for (int kx = 0; kx < vertexCount; ++kx)
			{
				...
				// 탄젠트 값 읽어오기.
				vertex.tangent = ReadTangent(pMesh, vertexIndex, vertexCounter);

				// 바이노멀 값 읽어오기.
				vertex.binormal = ReadBinormal(pMesh, vertexIndex, vertexCounter);
