...
void DXApp::InitProjectionMatrix(Shape::Mesh * mesh)
{
	...
	// 투영 행렬.
	projection = XMMatrixPerspectiveFovLH(fovY, (float)clientWidth / (float)clientHeight, nearZ, farZ);
}
...
void DXApp::InitMeshInfo()
{
	...
	sphereMesh.fbxName = "Resources//Model//sphere.fbx";
	...
	// 초기 트랜스폼 정보 설정.
	sphereMesh.scale = XMFLOAT3(100.0f, 100.0f, 100.0f);

	// 엔진에서 렌더링에 사용하는 메쉬 배열에 추가.
	...
	cardboardMesh.vertexShader = ShaderUtil::VertexShader(L"Shaders//ToonVS.fx", "main", "vs_5_0");
	cardboardMesh.pixelShader = ShaderUtil::PixelShader(L"Shaders//ToonPS.fx", "main", "ps_5_0");
	...
	// 메쉬 (하프 램버트 적용).
	Shape::Mesh halfDiffuseMesh;
	halfDiffuseMesh.fbxName = fbxNameTPP;
	halfDiffuseMesh.vertexShader = ShaderUtil::VertexShader(L"Shaders//HalfLabertianDiffuseVS.fx", "main", "vs_5_0");
	halfDiffuseMesh.pixelShader = ShaderUtil::PixelShader(L"Shaders//HalfLabertianDiffusePS.fx", "main", "ps_5_0");
	halfDiffuseMesh.pixelShader.pTextures.push_back(diffuseMap);
	halfDiffuseMesh.position = XMFLOAT3(0.0f, -120.0f, -30.0f);
	halfDiffuseMesh.rotation = XMFLOAT3(-90.0f, 180.0f, 0.0f);
	meshes.push_back(halfDiffuseMesh);
	...
	normalMesh.position = XMFLOAT3(150.0f, -120.0f, -30.0f);
	...
	cubeMesh.position = XMFLOAT3(300.0f, -120.0f, -30.0f);
	cubeMesh.rotation = XMFLOAT3(-90.0f, 180.0f, 0.0f);

	meshes.push_back(cubeMesh);

	ShaderUtil::Texture planeTex;
	planeTex.fileName = L"Resources//Textures//T_Ground_Grass_D.png";

	// 바닥 메쉬.
	Shape::Mesh planeMesh;
	planeMesh.fbxName = "Resources//Model//plane.fbx";

	planeMesh.vertexShader = ShaderUtil::VertexShader(L"Shaders//HalfLabertianDiffuseVS.fx", "main", "vs_5_0");
	planeMesh.pixelShader = ShaderUtil::PixelShader(L"Shaders//HalfLabertianDiffusePS.fx", "main", "ps_5_0");
	planeMesh.pixelShader.pTextures.push_back(planeTex);

	planeMesh.position = XMFLOAT3(0.0f, -120.0f, 0.0f);
	planeMesh.rotation = XMFLOAT3(-90.0f, 0.0f, 0.0f);
	planeMesh.scale = XMFLOAT3(100.0f, 100.0f, 100.0f);
