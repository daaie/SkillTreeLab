...
protected:
	...
	HRESULT LoadFBX(const char* fileName, std::vector<Vertex>* pOutVertex, std::vector<DWORD>* pOutIndex);

	// 정점 노멀 읽어오는 메소드.
	XMFLOAT3 ReadNormal(FbxMesh* pMesh, int controlPointIndex, int vertexCounter);

	XMFLOAT2 ReadUV(FbxLayerElementArrayTemplate<FbxVector2> *uvVertices, FbxMesh* pMesh, int polygonIndex, int vertexIndex);
};