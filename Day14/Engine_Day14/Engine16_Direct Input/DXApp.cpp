...
{
	// FBX 로드.
	HRESULT hr = LoadFBX("SK_Mannequin.fbx", &pVertices);
	if (FAILED(hr))
		return;
...
void DXApp::InitTransformMatrix()
{
	...
	// 카메라 위치.
	cameraPos = XMVectorSet(0.0f, 300.0f, -300.0f, 0.0f);
...
void DXApp::DetectInput(double deltaTime)
{
	...
	if (keyboardState[DIK_LEFT] & 0x80) rotZ -= 1.0f * deltaTime;
	if (keyboardState[DIK_RIGHT] & 0x80) rotZ += 1.0f * deltaTime;

	if (keyboardState[DIK_UP] & 0x80) rotX += 1.0f * deltaTime;
	if (keyboardState[DIK_DOWN] & 0x80) rotX -= 1.0f * deltaTime;

	// 회전 값이 0 ~ 360를 반복하도록 설정 (단위:라디안).
	if (rotX > 6.28f) rotX -= 6.28f;
	if (rotX < 0.0f) rotX = 6.28f + rotX;

	if (rotZ > 6.28f) rotZ -= 6.28f;
	if (rotZ < 0.0f) rotZ = 6.28f + rotZ;
